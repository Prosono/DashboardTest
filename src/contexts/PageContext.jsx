...
export const PageProvider = ({ children }) => {
  ...
  const [globalDashboardProfiles, setGlobalDashboardProfiles] = useState([{ id: 'default', name: 'default', updatedAt: null }]);
  const [globalStorageBusy, setGlobalStorageBusy] = useState(false);
  const [globalStorageError, setGlobalStorageError] = useState('');
  ...
  const refreshGlobalDashboards = async () => {
    setGlobalStorageError('');
    try {
      const profiles = await listSharedDashboards();
      setGlobalDashboardProfiles(profiles);
      return profiles;
    } catch (error) {
      console.warn('Failed to list global dashboards.', error);
      setGlobalStorageError('Unable to list global dashboards.');
      return [];
    }
  };
  ...
  const saveGlobalDashboard = async (profileId = 'default') => {
    ...
    try {
      ...
      setGlobalStorageError('Unable to save dashboard globally.');
      return false;
    } finally {
      setGlobalStorageBusy(false);
    }
  };
  ...
  const loadGlobalDashboard = async (profileId = 'default') => {
    ...
    try {
      ...
      if (!data) {
        setGlobalStorageError('Selected dashboard is empty or missing.');
        return false;
      }
      ...
      setGlobalStorageError('Unable to load selected global dashboard.');
      return false;
    } finally {
      setGlobalStorageBusy(false);
    }
  ...
  const value = {
    ...
    refreshGlobalDashboards: safeRefreshGlobalDashboards,
    saveGlobalDashboard: safeSaveGlobalDashboard,
    loadGlobalDashboard: safeLoadGlobalDashboard,
  };
