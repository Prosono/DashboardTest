...
export default function ModalOrchestrator({
  ...
  const editModalProps = useMemo(() => {
    ...
    const isEditSaunaMap = !!editId && editId.startsWith('sauna_map_card_');
    const isEditReports = !!editId && editId.startsWith('reports_card_');
    ...
    const isEditGenericType = (!!editSettings?.type && (editSettings.type === 'entity' || editSettings.type === 'toggle' || editSettings.type === 'sensor' || editSettings.type === 'divider' || editSettings.type === 'empty' || editSettings.type === 'calendar_booking' || editSettings.type === 'popup_launcher' || editSettings.type === 'notification_timeline' || editSettings.type === 'global_timeline' || editSettings.type === 'reports' || editSettings.type === 'sauna_health_score' || editSettings.type === 'sauna_map' || editSettings.type === 'alarmo' || editSettings.type === 'thermostat' || editSettings.type === 'input_text')) || isEditVacuum || isEditAutomation || isEditCar || isEditAndroidTV || isEditRoom || isEditSauna || isEditSaunaBookingTemp || isEditSaunaHealthScore || isEditSaunaMap || isEditReports || isEditThermostat || isEditAlarmo || isEditInputText || isEditPopupLauncher || isEditDivider || isEditEmpty || isEditCalendarBooking;
    ...
    const canEditIcon = !!editId && (
      isEditLight || isEditCalendar || isEditCalendarBooking || isEditTodo || isEditRoom || isEditSauna || isEditSaunaBookingTemp || isEditSaunaHealthScore || isEditSaunaMap || isEditReports || isEditCover || isEditDivider
      || editId.startsWith('automation.') || editId.startsWith('vacuum.')
      || editId.startsWith('climate_card_') || editId.startsWith('thermostat_card_') || editId.startsWith('cost_card_')
      ...
      || editId.startsWith('input_text_card_')
    ...
    return {
      ...
      isEditAndroidTV, isEditCar, isEditRoom, isEditSauna, isEditSaunaBookingTemp, isEditSaunaHealthScore, isEditSaunaMap, isEditReports, isEditThermostat, isEditAlarmo, isEditCamera, isEditInputText, isEditPopupLauncher, isEditDivider, isEditEmpty, isEditSensor, isEditWeatherTemp,
