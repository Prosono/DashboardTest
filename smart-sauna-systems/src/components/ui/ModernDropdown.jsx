...
export default function ModernDropdown({ label, icon: Icon, options, current, onChange, map, placeholder = "Not selected", onOpenChange }) {
  ...
  return (
    <div className={`relative w-full ${isOpen ? 'z-[120]' : ''}`} ref={dropdownRef}>
      <p className="text-xs uppercase font-bold mb-3 ml-1" style={{color: 'rgba(107, 114, 128, 1)', letterSpacing: '0.2em'}}>{label}</p>
      <button onClick={() => setOpen(!isOpen)} className="w-full flex items-center justify-between px-6 py-4 rounded-2xl group transition-all popup-surface popup-surface-hover">
      ...
      {isOpen && (
        <div className="absolute top-full left-0 w-full mt-2 z-[130] rounded-2xl overflow-hidden shadow-2xl border" style={{backgroundColor: 'var(--modal-bg)', borderColor: 'var(--glass-border)'}}>
          <div className="max-h-48 overflow-y-auto">
            {(options || []).map((option) => (
              <button key={option} onClick={() => { onChange(option); setOpen(false); }} className={`w-full text-left px-6 py-3 text-xs font-bold uppercase tracking-widest transition-all ${current === option ? 'text-blue-400' : 'text-[var(--text-secondary)] hover:bg-[var(--glass-bg-hover)] hover:text-[var(--text-primary)]'}`} style={{backgroundColor: current === option ? 'rgba(59, 130, 246, 0.1)' : 'transparent'}}>{String(getLabel(option))}</button>
            ))}
          </div>
        </div>
      )}
